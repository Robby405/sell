  ...
  methods: {
    drop(el) {
      ...
      for (let i = 0; i < this.balls.length; i++) {
        ...
        if (!ball.show) {
          ball.show = true; // 小球下落
          ball.el = el;
          this.dropBalls.push(ball);// 存已经下落的小球
          return;
        }
      }
    },
    ...
    beforeEnter(el) { // el为当前执行transition动画的DOM对象
      ...
      while (count--) {
        ...
        if (ball.show) { // 要运动的小球
          ...
          el.style.transform = `tramslate3d(0,${y}px,0)`;
    ...
    enter(el, done) {
      ...
      this.$nextTick(() => { // 样式重置
        ...
        el.style.transform = `tramslate3d(0,0,0)`;
        let inner = el.getElementsByClassName('inner-hook')[0];
    ...
    afterEnter(el) { // 动画完成
      // console.log('after enter'); 第一次不能触发afterenter
      let ball = this.dropBalls.shift(); // 删除并返回第一个ball
      if (ball) {
        ball.show = false; // 重置ball.show的的状态
